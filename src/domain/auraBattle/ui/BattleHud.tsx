...
export function BattleHud({ state, actions }: { state: BattleSnapshot; actions: BattleActions }) {
 ...
 const prompt = state.tutorial && state.tutorial !== "COUNTER"
  ? TUTORIAL[state.tutorial]
  : required && answering
   ? `${required} BEATS ${answering.toUpperCase()}`
   : yourTurn && state.phase === "FAIL"
    ? "PICK A RECOVERY"
    : yourTurn && state.phase === "FINAL_COUNTER"
     ? `STEAL ${answering?.toUpperCase() ?? "THE FINAL"}`
     : null;
  ...
  <div className="hud">
   <div className="hud-top">
    ...
    <div className="hud-center">
     {prompt ? <div className={`hud-cue${yourTurn ? " urgent" : ""}`}>{prompt}</div> : null}
   ...
   <div className="callouts">
    {state.callouts.slice(-1).map((callout) => (
