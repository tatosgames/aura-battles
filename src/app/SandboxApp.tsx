...
export function SandboxApp() {
  ...
  useEffect(() => { let alive = true; void lifecycle.current.init(); SandboxController.create(config).then((created) => { if (!alive) { created.dispose(); return; } controller.current = created; loop.current = new FixedStepLoop({ fixedUpdate: (dt) => created.step(dt), presentationUpdate: () => refresh(), onVisibilityChange: (hidden) => { if (hidden) void lifecycle.current.stopGameplay(); } }); void lifecycle.current.loadingFinished(); setReady(true); }); return () => { alive = false; loop.current = undefined; controller.current?.dispose(); void lifecycle.current.stopGameplay(); }; }, [refresh]);
