...
test("opens a duel, plays a card and moves the Aura meter", async ({ page }) => {
 ...
 await page.locator(".card-live").first().click();
...
test("plays a warmed match through to a Final Move and a rematch", async ({ page }) => {
 ...
 await expect(page.locator(".flame.on").first()).toBeVisible();
 ...
 while (Date.now() < deadline) {
  ...
  if (state.promptSide === 0) {
   if (state.canDeclareFinal) await page.locator(".primary.final").click({ timeout: 5_000 }).catch(() => undefined);
   else await page.locator(".card-live").first().click({ timeout: 5_000 }).catch(() => undefined);
  }
