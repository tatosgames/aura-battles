...
For high-frequency transforms, mutate Three object references inside `useFrame` from immutable snapshots or a small presentation store—do not enqueue broad React state updates every frame. A canvas is mounted only after `canCreateWebGLContext()` succeeds. Context loss/recovery and renderer configuration stay in `src/app/canvas`, not in gameplay code.
